...
public class EnderGuardLeggings extends ArmorMaterial {
    ...
    public void updateStats(UpdateStatsEvent e) {
        ...
        if (e.getPlayer().getWorldSD() instanceof TheEnd) {
            StatTypes.STATS.stream().filter(s -> s instanceof CombatStat).forEach(s -> e.getStats().addBaseMultiplier(s, 10, StatAdderType.ITEM, e.getPlayer().getItems().getLeggings()));
        }
    }
}
