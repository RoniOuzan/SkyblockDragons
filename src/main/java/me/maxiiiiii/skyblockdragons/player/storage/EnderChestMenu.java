...
public class EnderChestMenu extends Menu {
    ...
    @Override
    public void open() {
        if (getOwnerEnderChestSD().getOpener() != null && getOwnerEnderChestSD().getOpener() != player.getUniqueId()){
            player.sendMessage("§cThis chest is already open!");
            player.closeInventory();
            return;
        }
        getOwnerEnderChestSD().setOpener(player.getUniqueId());
        super.open();
    }
    ...
    public void update() {
        for (int i = 0; i < 45; i++) {
            this.setItem(i + 9, getOwnerEnderChestSD().getItems().get(i + (this.page * 45)));
        }
...
//        this.player.sendMessage(this.page);
    ...
    private EnderChest getOwnerEnderChestSD() {
        return this.owner.getEnderChestSD();
    }
    ...
    private void save() {
        for (int i = 0; i < 45; i++) {
            if (Functions.isNotAir(this.getInventory().getItem(i + 9))) {
                getOwnerEnderChestSD().setItem(i + (this.page * 45), this.getInventory().getItem(i + 9));
            } else {
                getOwnerEnderChestSD().removeItem(i + (this.page * 45));
            }
        }
        getOwnerEnderChestSD().setOpener(null);
    }
