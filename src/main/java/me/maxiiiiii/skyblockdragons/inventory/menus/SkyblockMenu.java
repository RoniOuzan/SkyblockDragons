...
public class SkyblockMenu extends Menu {
    ...
    @Override
    public void update() {
        this.setItem(13, Functions.applyHead(createItem(Material.SKULL_ITEM, 3, ChatColor.GREEN + "Your Skyblock Profile", "STATS", player.getStats().stream().map(s -> s.getType() + " " + ChatColor.WHITE + Functions.getNumberFormat(s.get())).collect(Collectors.toList())), this.player.getPlayer()));
...
//            player.performCommand("settings");
    ...
    public static class Stats extends Menu {
        ...
        @Override
        public void update() {
            this.setItem(4, Functions.applyHead(createItem(Material.SKULL_ITEM, 3, ChatColor.GREEN + "Your Skyblock Profile", "STATS", player.getStats().stream().map(s -> s.getType() + " " + ChatColor.WHITE + Functions.getNumberFormat(s.get())).collect(Collectors.toList())), this.player.getPlayer()));

            int i = 0;
            for (Stat stat : this.player.getStats()) {
                this.setItem(Functions.intToSlot(i), createItem(stat.getType().getItem().getType(), stat.getType().getItem().getDurability(), stat.getType() + " " + Functions.getNumberFormat(stat.get()), stat.getType().getName().toUpperCase().replace(" ", "_")));
                if (stat.getType() == StatTypes.DEFENSE) {
                    this.setItem(Functions.intToSlot(i), Functions.setLore(this.getItem(Functions.intToSlot(i)), ChatColor.GRAY + "Effective Health: " + ChatColor.GREEN + Functions.getNumberFormat(player.getStats().get(StatTypes.HEALTH).get() * (1 + (stat.get() / 100)))));
                }
                i++;
            }
        }
